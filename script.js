...
// Feature items
document.querySelectorAll('.fi').forEach(i=>{i.addEventListener('click',()=>{const l=i.closest('.fl');l.querySelectorAll('.fi').forEach(x=>x.classList.remove('active'));i.classList.add('active');const s=i.dataset.img;if(s){const img=i.closest('.fg').querySelector('.fd img');img.style.opacity='0';setTimeout(()=>{img.src=s;img.alt=i.textContent.trim();img.style.opacity='1'},150)}});});
