...
/**
 * Google Sheet I/O via a deployed Apps Script Web App.
 * The Apps Script source lives in `docs/google-apps-script.gs`.
 *
 * - Reading pending rows is REQUIRED for syncing new work (callers should
 *   handle failures by falling back to already-synced ledger jobs).
 * - Write-back and alerts are BEST-EFFORT (human visibility only): they never
...
/**
 * Read rows that still need processing (sheet status empty or "pending").
 * Throws on failure — the caller decides whether to continue with already-synced jobs.
 */
...
/** Best-effort: email a summary of newly-failed jobs (via Apps Script MailApp). Never throws. */
