...
/**
 * Publish a generated article directly to Supabase (service role).
 * Resolves system author, category, tags, ensures slug uniqueness,
 * inserts a published post, links tags, and revalidates public routes.
 */

const SYSTEM_AUTHOR_SLUG = 'revue-editorial';
...
function mergeTagNames(articleTags: string[], sheetTags?: string | null): string[] {
  const fromSheet = (sheetTags ?? '')
    .split(',')
    .map((t) => t.trim())
    .filter(Boolean);
  return [...articleTags, ...fromSheet];
}
...
/**
 * Insert a published post and return its public URL.
 * `contentHtml` must already be sanitized (see sanitize.ts).
 */
export async function publishGeneratedPost(params: {
  article: GeneratedArticle;
  contentHtml: string;
  categoryName?: string | null;
  sheetTags?: string | null;
  featuredImageUrl?: string | null;
}): Promise<PublishedPostResult> {
  const { article, contentHtml, categoryName, sheetTags, featuredImageUrl } = params;
  ...
  const tagIds = await resolveTagIds(mergeTagNames(article.tags, sheetTags));
